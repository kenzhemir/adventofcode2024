...
console.log(findNoPath());

//-----------

function findNoPath(input = parseInput()) {
  return input[
    binarySearch(
      stepLimit,
      input.length,
      (steps) => !(fallingSim(input, steps) < Infinity)
    ) - 1
  ];
}

function fallingSim(input = parseInput(), stepLimit = stepLimit) {
